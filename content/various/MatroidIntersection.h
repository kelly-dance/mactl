 ...
 * Time: O(R^2 N(M2.add + M1.check + M2.check) + R^3 M1.add + R^2 M1.clear + RN M2.clear)
 * Status: Tested on 1284G, 102156D
 */
#pragma once
...
struct GraphMat {
	UF uf;
	vector<pii> e;
	GraphMat(int n, vector<pii> e) : uf(n), e(e) {}
	bool check(int x) { return !uf.sameSet(e[x].F, e[x].S); }
	void add(int x) { uf.join(e[x].F, e[x].S); }
